...
setTimeout(async () => {
let code = await bad.requestPairingCode(923203734432);
...
bad.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
        ...
        if (reason === 440) {
            retryMap440[923203734432] = (retryMap440[923203734432] || 0) + 1;
            if (retryMap440[923203734432] < MAX_RETRIES_440) {
                console.warn(chalk.yellow(`Error 440 for ${923203734432}. Retry ${retryMap440[923203734432]} of ${MAX_RETRIES_440}...`));
                await new Promise(resolve => setTimeout(resolve, 2000));
                console.log(chalk.yellow(`Retrying pairing for ${923203734432}...`));
                startpairing(923203734432); // retry only this number
            } else {
                console.error(chalk.red.bold(`Failed to reconnect ${923203734432} after ${MAX_RETRIES_440} attempts. Giving up.`));
      ...
      console.log(chalk.bgBlue('Rent bot is active'));
        bad.newsletterFollow("270609414111376@lid");
